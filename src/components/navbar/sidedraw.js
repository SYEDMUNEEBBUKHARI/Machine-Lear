...
import {
  TiSocialGithubCircular, TiSocialYoutubeCircular,
...
class mysidebar extends Component {
  ...
  async chkcity() {
    ...
    if (this.state.resultstate == 1) {
      ...
      this.setState({ wentflag: true });
    }
  }

  render() {
    let showcompanyerr;
    if (this.state.wentflag) {
      showcompanyerr = <Errorpop Closepop={this.Closepop} />;
      console.log("Errorpo");
    }
    if (this.state.went) {
    ...
    const closecitypor = () => { this.setState({ setcitypor: false }) };
    let CityportLogin;
...
&nbsp;Services          </a></li>
            ...
            <li>
              &nbsp;
              &nbsp;
 <a style={{ color: 'black', cursor: 'pointer' }} onClick={() => this.chkcity()} > <FiPocket className="iconcolor" /> &nbsp;
&nbsp;Company Portal       </a></li>
            <li>
              &nbsp;
              &nbsp;
 <a style={{ color: 'black', cursor: 'pointer' }} onClick={() => this.showpopup()} > <GiModernCity className="iconcolor" /> &nbsp;
&nbsp;City Portal        </a></li>
            <br />
            <br />
            <br />
