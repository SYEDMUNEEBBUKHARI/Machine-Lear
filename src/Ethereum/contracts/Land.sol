...
contract LandOwnership {
    function superuser() public view returns (uint256) {
    ...
    mapping(bytes32 => uint256) whtidis;
    ...
    function LandRegistration(
        ...
        require(
            cityappmeta[msg.sender].cityapp[city] == true,
            "this is not city's portal id"
        );
        require(!(serialexist[ipfsh] == true), "serail already exist");
        bytes32 d = sha256(
            abi.encodePacked("0x", completeaddress, metaid, count++)
        );
        require(
        ...
        serialNoverification[d] = true;
        serialexist[ipfsh] = true;
        return true;
    }

    function Cityandusers(string memory city)
        public
        view
    ...
    function landInfoOwner(bytes32 enterid)
        public
        view
        returns (
            bytes32,
    ...
    {
        return (
            Land[enterid].serialNo,
            Land[enterid].currentOwner,
            Land[enterid].completeaddress,
            Land[enterid].LandArea,
            Land[enterid].LandSaleable
        );
    }
    ...
    bytes32[] memsale;
    mapping(bytes32 => uint256) saleableid;

    function makeSaleable(bytes32 property) public returns (uint256) {
        require(Land[property].currentOwner == msg.sender);
        Land[property].LandSaleable = true;
        memsale.push(property);
        uint256 Landidlength = memsale.length - 1;
    ...
    function deletesaleable(bytes32 lname) private returns (bool) {
        ...
        uint256 Landid = saleableid[lname];
        uint256 idd = memsale.length - 1;
    ...
    mapping(bytes32 => uint256) appnotiid;
    ...
    function makeApproved(bytes32 Landid, address buyer) public returns (bool) {
        ...
        uint256 takelength = appnotifi[buyer].AddLand.length - 1;
    ...
    function BuyLand(bytes32 Landid) public returns (bool) {
        ...
        //last property id
        profile[previousOwner].noOfLand[findid] = profile[previousOwner]
            .noOfLand[profile[previousOwner].noOfLand.length - 1];
        whtidis[lastproperty] = findid;
        delete profile[previousOwner].noOfLand[profile[previousOwner]
            .noOfLand
            .length - 1];
        ...
        uint256 index = appnotiid[Landid];
        appnotifi[msg.sender].AddLand[index] = appnotifi[msg.sender]
            .AddLand[appnotifi[msg.sender].AddLand.length - 1];
        delete appnotiid[Landid];
        delete appnotifi[msg.sender].AddLand[appnotifi[msg.sender]
            .AddLand
            .length - 1];
    ...
    mapping(address => uint256) approverid;
    ...
    function registerapprover(
        ...
        require(
            msg.sender == 0xa2C1ee3dd1ac4b8Ed475396Fd1EF044Bcd25A40A,
        ...
        uint256 apid = approverlist.length - 1;
    ...
    function viewapprover(string memory city)
        public
        view
        returns (
            string memory,
        ...
        )
    {
        ...
        return (
            makeapprover[city].phno,
            makeapprover[city].city,
            makeapprover[city].email,
            makeapprover[city].metaid
        );
    }
    ...
    function cityLand(string memory city)
        public
        view
        returns (bytes32[] memory)
    {
    ...
    function cityverify(address meta, string memory cicity)
        public
        returns (uint256)
    {
